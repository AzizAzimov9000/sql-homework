...
FROM 
Products CROSS JOIN Suppliers

select 
DepartmentName, Name from
...
SELECT 
    p.ProductName, o.OrderID, o.CustomerID, o.Quantity FROM 
 ...
 select e.Name, d.DepartmentName
  ...
  select * from Orders o join Products p on o.ProductID=o.ProductID
  where p.price > 100
  ...
  select c.FirstName, s.ProductID from Customers c 
  join Sales s on c.CustomerID=s.CustomerID
  where s.SaleAmount>=500

  select s.Name, c.CourseName from Courses c join Enrollments e on c.CourseID=e.CourseID
  ...
  select d.DepartmentName, e.Name from Employees e join Departments d on e.DepartmentID=d.DepartmentID
  select * from Products , Categories

  select p.ProductName, c.CategoryName from Products p join Categories c on p.SupplierID=c.CategoryID
  where c.CategoryName in ('Electronics', 'Furniture')
   ...
   select s.SaleID,
    ...
    s.SaleDate,
    c.FirstName,
    c.Country from Sales s join Customers c on s.CustomerID=c.CustomerID
   where c.Country = 'USA'
   ...
   select s.StudentID, s.Name from Courses c join Enrollments e on c.CourseID=e.CourseID
  join Students s on s.StudentID=e.StudentID
...
WHERE 
    ...
    c.FirstName,
    o.OrderID,
    o.Quantity from Payments p join Orders o on p.OrderID=o.OrderID 
	join Customers c on c.CustomerID=o.CustomerID
	where o.Quantity=1 and p.PaymentID is null

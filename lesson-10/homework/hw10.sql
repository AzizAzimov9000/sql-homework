...
select e.name, e.salary, d.DepartmentName as EmployeeName  
...
select e.Name as EmployeeName, d.DepartmentName  from Employees e LEFT JOIN Departments d on e.departmentID=d.departmentID

select s.SupplierName, p.ProductName from Products p  LEFT JOIN  Suppliers s on p.SupplierID=s.SupplierID

select o.OrderID, o.OrderDate, p.PaymentDate, p.Amount from Orders o FULL OUTER JOIN Payments p on o.orderid=p.orderid

SELECT e.Name AS EmployeeName, m.Name AS ManagerName FROM 
Employees e left JOIN Employees m ON e.ManagerID = m.EmployeeID

select s.Name as StudentName, c.CourseName from Students s join Enrollments e on s.studentid=e.studentid
...
select d.DepartmentName, count(e.name) as EmployeeCount from Employees e join Departments d on e.DepartmentID=d.DepartmentID
...
select  e.Name AS EmployeeName, d.DepartmentName from Employees e  join Departments d on e.DepartmentID=d.DepartmentID
...
select p.ProductID, p.ProductName from Products p left join Orders o on p.productid=o.productid
WHERE 
    o.OrderID IS NULL
...
select o.OrderID, o.OrderDate from Orders o left join Payments p on o.orderid=p.orderid
where year(p.paymentdate)<2020

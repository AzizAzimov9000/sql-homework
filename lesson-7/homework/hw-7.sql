...
select sum(SaleAmount) AS total_sales, AVG(SaleAmount) AS average_sales from sales

select count(EmployeeID) AS NumberOfEmployees from Employees

select DepartmentName, MAX(Salary) AS highest_salary,
...
select DepartmentName, avg(salary) as AvgSalary, count(*) from Employees as EmployeeCount
group by DepartmentName
...
select sum(EmployeeID) as EmployeeID, avg(salary) as avgsalary, departmentname from Employees
group by departmentname
having avg(salary) >65000

select sum(saleamount), count(ProductID), SaleDate from Sales
group by SaleDate


select max(Quantity) as maxsold, min(Quantity) as minsold, YEAR(OrderDate) from Orders

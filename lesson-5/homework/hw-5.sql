...
select IIF( StockQuantity>100, 'low', 'no') as Stock from Products_Discounted
...
select *,
case when DepartmentName = 'HR' or EmployeeID = 5 then
'salary'*1.10  else 0 end 'newsalary' from Employees
...
case when SaleAmount > 500 then 'Top Tier'
   ...
   else 'Low Tier' end performamce from Sales

ALTER TABLE Sales
ALTER COLUMN SaleAmount INT
